...
void Board::loadSettings(Monitor *monitor)
{
    ...
    serial_log_size_ = db_.get(
        "serialLogSize",
        static_cast<qulonglong>(monitor ? monitor->serialLogSize() : 0)).toULongLong();
...
void Board::setSerialLogSize(size_t size)
{
    ...
    db_.put("serialLogSize", static_cast<qulonglong>(size));
    emit settingsChanged();
}
...
void Board::serialReceived(ty_descriptor desc)
{
    ...
    // NOTE: echo serial input in serial log too
...
void Board::updateSerialLogState()
{
    ...
    if (serial_log_size_) {
        ...
        if (serial_log_file_.isOpen() && serial_log_file_.size() > static_cast<qint64>(serial_log_size_))
